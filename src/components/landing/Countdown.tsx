  ...
  return (
    <div className="flex flex-col items-center">
      <div
        ...
        className={`min-w-[3.6rem] rounded-xl border border-gold/40 bg-wine-deep/60 px-2 py-2.5 text-center font-display text-3xl font-bold tabular-nums text-background sm:min-w-[4.5rem] sm:text-4xl ${animate ? "animate-tick" : ""}`}
      >
        {value}
      </div>
      <span className="mt-1.5 eyebrow text-[0.6rem] text-gold">{label}</span>
    </div>
  );
}
...
export function Countdown({ compact = false }: { compact?: boolean }) {
  ...
  if (expired) {
    return (
      <p className="mx-auto max-w-md text-sm font-semibold text-background/90">
  ...
  return (
    ...
    >
      ...
      <span className="pt-2 font-display text-3xl text-gold">:</span>
